...
//	}
	...
	*/
	...
	/*
		This is a method to check if a piece is a Black or White piece.
	*/
		...
		*/
			...
			if (pieceName.equals("BlackPawn")) {
				...
				if ((xMovement == 0) && (yMovement == 0)) {
					validMove = false;
				} else if ((startY == 6) && (startX == landingX) && (((startY - landingY) == 1) || (startY - landingY) == 2)) {
					if (!piecePresent(e.getX(), e.getY())) {
						validMove = true;
					} else {
						validMove = false;
					}
					if (piecePresent(e.getX(), initialY - (1 * 75))) {
						inTheWay = true;
					} else {
						inTheWay = false;
					}
			...
			if (pieceName.contains("BlackKing")) {
				...
				//This part makes sure the kings cannot come within a square of eachother by checking surrounding sqares
			...
			if (pieceName.contains("BlackBishop")) {
				Boolean inTheWay = false;
				int distance = Math.abs(startX - landingX);
				if ((xMovement == 0) && (yMovement == 0)) {
					validMove = false;
			...
			if (pieceName.contains("BlackKnight")) {
				if ((xMovement == 0) && (yMovement == 0)) {
					validMove = false;
...
//				if ((xMovement == 0) && (yMovement == 0)) {
//					validMove = false;
//				} else if ((startY == 1) && (startX == landingX) && (((startY - landingY) == -1) || (startY - landingY) == -2)) {
//					if (!piecePresent(e.getX(), e.getY())) {
//						validMove = true;
//					} else {
//						validMove = false;
...
*/
  ...
  */
    ...
    */
	...
	//Moves for the white pawn - note to self: pawn moves (Down one, Down two, down and right one, down and left one)
	private Stack getWhitePawnSquares(int x, int y, String piece){
		...
		if(y == 1){
			...
			if(!piecePresent(((tmp1.getXC()*75)+20), (((tmp1.getYC()*75)+20))) && !piecePresent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){
				moves.push(validM4);
			}
			else{
				if (!(tmpx1 > 7)){
			...
			if(l instanceof JLabel){
				...
				if(c instanceof JLabel) {
					if (startingPoint.getName().contains("Pawn") && landingPoint.getYC() == 7) {
						...
						pieces = new JLabel(new ImageIcon("WhiteQueen.png"));
						parent = (JPanel) chessBoard.findComponentAt(landingX1, landingY1);
						parent.add(pieces);
					}
				}
